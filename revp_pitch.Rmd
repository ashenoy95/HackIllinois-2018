...
ggplot(avg_days_per_place[1:10,]) + 
...
```{r warning=FALSE, message=FALSE}
leaflet(champaign_closed_wgs84) %>% addTiles() %>% 
...
```{r warning=FALSE, message=FALSE}
leaflet(champaign_open_wgs84) %>% addTiles() %>%
...
leaflet(champaign_closed_sorted_wgs84[1:20,]) %>% addTiles() %>% 
